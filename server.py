...
# TODO: Add summary
def GetLeadInfo(cookie_dict, lead_profile, profile_urn):
    ...
    prompt = "This is the profile of a person: " + lead_profile + \
...
def get_lead_info():
    ...
    for lead_idx, profile_urn in enumerate(member_urn_id_list):
        ...
        if lead_idx == 2:
            break    
        data = q.enqueue(GetLeadInfo, cookie_dict, leads_list[lead_idx][0], profile_urn)
        job_id = data.get_id()
        job_id_list.append(job_id)
    # data = GetLeadInfo(cookie_dict, leads_list, member_urn_id_list)

    return jsonify(success=True, message=job_id_list)
